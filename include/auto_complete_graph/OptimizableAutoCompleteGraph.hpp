...
class OptimizableAutoCompleteGraph : public g2o::SparseOptimizer {
   ...
   public:
    ...
    // TODO : attention -> make sure that the initializeOptimization does not
    // reinitialize the state of the graph as in g2o_viewer
    ...
    /// ATTENTION Call setHuberKernel are other Kernel first ! It's not in the
    /// function here just so that I can use a different kernel function in
    /// class that inherite this one. I know that design suck. I should inherite
    ...
    void setHuberKernel() {
        ...
        for (auto ite = idmapedges.begin(); ite != idmapedges.end(); ++ite) {
            OptimizableGraph::Edge* e =
                static_cast<OptimizableGraph::Edge*>(*ite);
    ...
    void removeRobustKernel() {
        ...
        for (SparseOptimizer::EdgeSet::const_iterator it = edges().begin();
             it != edges().end(); ++it) {
            OptimizableGraph::Edge* e =
                static_cast<OptimizableGraph::Edge*>(*it);
            e->setRobustKernel(0);
        }
    }
   ...
   private:
    void setRobustKernelAllEdges(g2o::RobustKernel* ptr = NULL,
                                 double width = 1);
};
}  // namespace acg
}  // namespace AASS
